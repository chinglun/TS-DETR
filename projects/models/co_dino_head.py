...
class CoDINOHead(CoDeformDETRHead):
    ...
    def forward(self,
        ...
        for feat in mlvl_feats:
            ...
            # print(mlvl_positional_encodings)
            # input("")
        # input("")
        query_embeds = None
        ...
        # print(topk_anchor.shape)#torch.Size([1, 900, 4])
        # print(topk_score.shape)#torch.Size([1, 900, 98])
        # input("")
        # search for each index and max score index in top-k
        # query_indices_x1 = topk_anchor[:, :, 0]
        # query_indices_y1 = topk_anchor[:, :, 1]
        # query_indices_x2 = topk_anchor[:, :, 2]
        # query_indices_y2 = topk_anchor[:, :, 3]
        # class_indices = topk_score.argmax(dim=-1)

        # # print score and index
        # for i in range(topk_score.shape[0]):
        #     print(f"Image {i + 1}:")
        #     for j in range(20): topk
        #         query_index_x1 = query_indices_x1[i, j].item()
        #         query_index_y1 = query_indices_y1[i, j].item()
        #         query_index_x2 = query_indices_x2[i, j].item()
        #         query_index_y2 = query_indices_y2[i, j].item()
        #         class_index = class_indices[i, j].item()
        #         max_score = topk_score[i, j, class_index].item()
        #         with open("visualize_query/baseline_detection.txt", 'a') as f:
        #             f.write(f"{class_index} {query_index_x1} {query_index_y1} {query_index_x2} {query_index_y2}\n")
        #         #print(f"Top-{j + 1} max score (query index {query_index_x1} {query_index_y1} {query_index_x2} {query_index_y2}): Class {class_index} with score {max_score}")
        # #input("")
        outs = []
        ...
        #     feat_map = cv2.resize(feat_map, (3840, 2160), interpolation=cv2.INTER_LINEAR)
        #     if i == 0:
        #         attn_map = feat_map
        #     else:
        #         attn_map += feat_map
        ...
        # print(type(attn_map))
        # plt.title('Heatmap of 2048x2048 Array')
        # plt.xlabel('X-axis')
        # plt.ylabel('Y-axis')
    ...
    def loss(self,
        ...
        if enc_topk_scores is not None:
            ...
            # num=enc_loss_cls+enc_losses_bbox+enc_losses_iou
            # loss_dict['enc_loss_cls'] = (5*enc_loss_cls/num+1)*enc_loss_cls/num
            # loss_dict['enc_loss_bbox'] = (5*enc_losses_bbox/num+1)*enc_losses_bbox/num
            # loss_dict['enc_loss_iou'] = (5*enc_losses_iou/num+1)*enc_losses_iou/num
            ...
            # if enc_loss_cls>enc_losses_bbox:
            #     a=0.5
            #     b=5.5
            #     c=2
            # else:
